...
SITE = {
    ...
    "paginate": 6,
    # Cache-buster for style.css: a hash of the SCSS sources, so the query
    # string only changes when the stylesheet actually changes.
    "css_version": "",
}
...
def scss_version():
    import hashlib
    h = hashlib.sha1()
    for d in ("_sass", os.path.join("assets", "css")):
        base = os.path.join(ROOT, d)
        for name in sorted(os.listdir(base)):
            if name.endswith(".scss") and not name.startswith("."):
                with open(os.path.join(base, name), "rb") as f:
                    h.update(f.read())
    return h.hexdigest()[:8]


SITE["css_version"] = scss_version()
